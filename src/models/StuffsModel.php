...
class StuffsModel {
  ...
  public function addStuff($data) {
    ...
    $this->db->query('INSERT INTO stuffs (id, category_id, user_id, name, description, created_at, updated_at) VALUES (:id, :category_id, :user_id, :name, :description, :created_at, :updated_at)');
    $this->db->bind('id', $id);
  ...
  public function updateStuff($data) {
    ...
    $this->db->query('UPDATE stuffs SET category_id = :category_id, name = :name, description = :description, updated_at = :updated_at WHERE id = :id');
    $this->db->bind('category_id', $data['category']);
    $this->db->bind('name', $data['name']);
    $this->db->bind('description', $data['description']);
    $this->db->bind('updated_at', $updatedAt);
